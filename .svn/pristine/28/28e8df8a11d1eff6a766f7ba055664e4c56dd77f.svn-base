...
public class StateRepository extends BaseRepository<State>{
    ...
    protected List<StateInfoWithIdeaCount> getStatesForDropDown(Session sess) throws HibernateException{
        ArrayList<StateInfoWithIdeaCount> res= (ArrayList<StateInfoWithIdeaCount>) sess.createQuery("select new rzd.vivc.ideax.model.pageobjeсt.StateInfoWithIdeaCount(s.name, SIZE(s.ideas), s.id) from State as s where s.deleted=0 group by s").list();
res.addAll((ArrayList<StateInfoWithIdeaCount>) sess.createQuery("select new rzd.vivc.ideax.model.pageobjeсt.StateInfoWithIdeaCount(s.name, 0, s.id) from State as s where s.deleted=0 AND NOT EXISTS(SELECT i.id from Idea as i where i.state=s)").list());
        return res;
    }
