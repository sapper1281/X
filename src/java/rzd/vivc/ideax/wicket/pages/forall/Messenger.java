...
public final class Messenger extends IdeaPage {
    ...
    public Messenger() {
        ...
        add(new BookmarkablePageLink("exit", Login.class));
        add(reload);
        ...
        reload.add(new RefreshingView("ideas") {
            ...
            protected void populateItem(Item item) {
                ...
                DirectorsAddLink edit;
                BookmarkablePageLink comment;
                //куда переходить при клике на идею

                edit = new DirectorsAddLink("edit", Input.class, new PageParameters().add("id", idea.getId()));
                comment = new BookmarkablePageLink("comment", Recenzia.class, new PageParameters().add("id", idea.getId()));
                item.add(edit);
                item.add(comment);
                //тема
                item.add(new Label("theme", idea.getTheme()));
                //текст
                item.add(new MultiLineLabel("text", idea.getText()));
                //ФИО
                item.add(new Label("fio", idea.getUserCreator().getFIO()));
                //дата до которой выполнить
                item.add(new DateLabel("date", idea.getDt_end()));
            }
        });
    }

    //ссылка на переход на страницу редктировагие, лоступная только пользователям с правами директора
    @DirectorOnly
